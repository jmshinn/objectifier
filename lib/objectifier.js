...
if (typeof setImmediate === 'function') nextLoop = setImmediate;
else nextLoop = process.nextTick;
...
exports.objectify = function(str) { // str is the only required argument
	...
	if (typeof str === 'object') {
		if (callback) nextLoop(function() { callback(null, str); });
		else return str;
		return;
	}
	// otherwise, if it's not a string, it's an error
		...
		if (callback) nextLoop(function() { callback(err); });
		else return err;
	}
	// trivially handle empty inputs
	if (!str.trim()) {
		if (callback) nextLoop(function() { callback(null, {}); });
		else return {};
		return;
	}
	...
	if (format == 'querystring') {
		...
		if (callback) nextLoop(function() { callback(null, forceobj(qs.parse(str))); });
		...
		if (callback) {
			nextLoop(function() {
				var obj, err=null;
				try {
					obj = forceobj(JSON.parse(str));
				}
				catch (e) {
					// we specifically requested json
					if (typematch == 1 && qstring_allowed != 1) err = new Error('JSON was specified as the string format, but failed to parse correctly');
					else obj = forceobj(qs.parse(str));
				}
				callback(err, obj);
			});
		}
...
exports.stringify = function(obj) { // obj is required
	...
	if (typeof obj === 'string') {
		if (callback) nextLoop(function() { callback(null, obj); });
		else return obj;
	}
	...
	if (format == 'querystring') {
		...
		// supports querystring, qstring, query
		if (callback) nextLoop(function() { callback(null, qs.stringify(obj)); });
		else return qs.stringify(obj);
	}
	else if (format == 'json') {
		if (callback) nextLoop(function() { callback(null, JSON.stringify(obj)); });
		else return JSON.stringify(obj);
	}
	else if (format == 'xml') {
		xmlbuilder.options.renderOpts.pretty = false;
		if (callback) nextLoop(function() { callback(null, xmlbuilder.buildObject(obj)); });
